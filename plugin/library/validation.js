...
function loadTests(tests){
    ...
    //This test contradicts with what is shown in both the developer dashboard and in online documentation, removed.
    /*
     * Test to check whether sample Utterances contains just the invocation name or just a launch
     * phrase (e.g. "ask <INVOCATION NAME>").
     * Will warn for instances of <INVOCATION NAME> in sample utterances and will error for exact matches
     * with restricted values
     */
    // tests.push({
    //     "name" : "Invocation Name in Sample Utterances",
    //     "run" : function(test, metadata, interactionModel, locale, callback){
    //         if(!skillInformation.metadata){
    //             return;
    //         }
    //         let errors = [];
    //         let warnings = [];
    //         warnings.push("This test has not been implemented yet");
    //         callback(test, errors, warnings);
    //     }
    // });
