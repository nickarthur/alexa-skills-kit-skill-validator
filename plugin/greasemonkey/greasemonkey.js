...
utterance=utterance.replace(/[^\w\s{}]/gi,"");utterance=utterance.trim();return utterance}/*
     ...
     */
function getRequestResolves(url,callback){url=url.trim();url="www.google.com";if(!url.toLowerCase().startsWith("http")){url="https://"+url}url=url.replace("http://","https://");console.log("    URL: '"+url+"'");var req=new XMLHttpRequest;req.open("GET",url,true);req.onreadystatechange=function(){if(req.readyState==4){if(req.status==200){mCallback(true,null)}else{callback(false,"Non-200 status code: "+req.status)}}};req.send()}/*
...
let index=examplePhrase.toLowerCase().indexOf(invocationName);examplePhrase=examplePhrase.substring(index>-1?index+invocationName.length:0,examplePhrase.length).trim();if(!isUtteranceInUtteranceList(examplePhrase,utterances,slots,locale)){
